...
const io = new Server(server, { // 4. Inisialisasi socket.io
    cors: {
...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    // Membersihkan nama file: ganti spasi dengan '-', hapus karakter aneh
...
app.post('/api/playlists/:playlistId/media', async (req, res) => {
  ...
  if (!playlist || !media) return res.status(404).send('Playlist atau Media tidak ditemukan');
...
app.put('/api/playlists/:id', async (req, res) => {
  try {
        ...
        for (const item of items) {
            ...
            if (media) {
                await playlist.addMedia(media, { through: { duration: item.duration } });
            }
        }

        // --- TAMBAHAN LOGIKA SOCKET.IO ---
        // Cari semua device yang menggunakan playlist ini
        const devicesToUpdate = await Device.findAll({ where: { PlaylistId: req.params.id } });
        ...
        res.status(500).json({ message: 'Gagal memperbarui playlist', error: error.message });
    }
});
...
app.delete('/api/media/:id', async (req, res) => {
  ...
  console.log("ID yang diterima dari URL:", idFromUrl, "| Tipe data:", typeof idFromUrl);
  try {
...
app.delete('/api/playlists/:playlistId/media/:mediaId', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Gagal menghapus item dari playlist.', error });
  }
});
...
app.put('/api/devices/:id/assign', async (req, res) => {
    try {
        ...
        console.log(`Menugaskan playlist, mengirim sinyal update ke room: ${device.deviceId}`);
        io.to(device.deviceId).emit('playlist:updated');
        // -------------------------------

        res.json(device);
    } catch (error) {
        res.status(500).json({ message: 'Gagal menugaskan playlist', error: error.message });
    }
});
...
app.get('/api/player/:deviceId', async (req, res) => {
  try {
    ...
    if (!device || !device.Playlist) {
      return res.status(404).json({ message: 'Tidak ada playlist yang ditugaskan.' });
    }
...
sequelize.sync({ alter: true }).then(() => {
    ...
    server.listen(port, () => {
        console.log(`Server (termasuk Socket.io) berjalan di http://localhost:${port}`);
    });
});