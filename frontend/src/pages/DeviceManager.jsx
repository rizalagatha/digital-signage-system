...
function DeviceManager() {
    ...
    const handleAssignPlaylist = async (deviceId, playlistId) => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/devices/${deviceId}/assign`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    const getDeviceStatus = (lastSeen) => {
        if (!lastSeen) return { text: 'Offline', color: '#6c757d' };
    ...
    return (
        <div className="device-section" style={{ padding: '20px', background: 'white', borderRadius: '8px' }}>
            <h2>Manajemen Perangkat TV</h2>

            {/* Form untuk mendaftarkan perangkat baru */}
            <form onSubmit={handleRegisterDevice} style={{ marginBottom: '20px', display: 'flex', gap: '10px', alignItems: 'center' }}>
                <input
                    type="text"
                ...
                <input
                    ...
                    required
                />
                <button type="submit" style={{ padding: '8px 16px', border: 'none', background: '#007bff', color: 'white', borderRadius: '4px', cursor: 'pointer' }}>Daftarkan</button>
            </form>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                <thead>
                    <tr style={{ backgroundColor: '#f2f2f2', textAlign: 'left' }}>
                        ...
                        <th style={{ padding: '10px', border: '1px solid #ddd' }}>Nama Perangkat</th>
                        <th style={{ padding: '10px', border: '1px solid #ddd' }}>ID Unik</th>
                        <th style={{ padding: '10px', border: '1px solid #ddd' }}>Terakhir Dilihat</th>
                        <th style={{ padding: '10px', border: '1px solid #ddd' }}>Playlist Aktif</th>
                        <th style={{ padding: '10px', border: '1px solid #ddd', width: '200px' }}>{/* Tambah width */}
                            Tugaskan Playlist
                        </th>
                        ...
                        return (
                            <tr key={device.id}>
                                <td style={{ padding: '10px', border: '1px solid #ddd', textAlign: 'center', fontWeight: 'bold' }}>
                                    <span style={{ height: '10px', width: '10px', backgroundColor: status.color, borderRadius: '50%', display: 'inline-block', marginRight: '8px' }}></span>
                                    {status.text}
                                </td>
                                <td style={{ padding: '10px', border: '1px solid #ddd' }}>{device.name}</td>
                                <td style={{ padding: '10px', border: '1px solid #ddd' }}><code>{device.deviceId}</code></td>
                                <td style={{ padding: '10px', border: '1px solid #ddd' }}>{device.lastSeen ? new Date(device.lastSeen).toLocaleString('id-ID') : 'Belum pernah'}</td>
                                <td style={{ padding: '10px', border: '1px solid #ddd' }}>{device.Playlist ? device.Playlist.name : '–'}</td>
                                <td style={{ padding: '10px', border: '1px solid #ddd', whiteSpace: 'nowrap' }}> {/* Tambah white-space */}
                                    <select
                                        onChange={(e) => handleAssignPlaylist(device.id, e.target.value)}
                                        value={device.PlaylistId || ''}
                                        style={{ padding: '5px', width: 'calc(100% - 70px)' }} /* Kurangi lebar untuk memberi ruang */
                                    >
                                        <option value="">- Lepaskan Playlist -</option>
                                        {playlists.map(pl => <option key={pl.id} value={pl.id}>{pl.name}</option>)}
                                    </select>
                                </td>
                                <td style={{ padding: '10px', border: '1px solid #ddd', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}> {/* Tambah flex layout */}
                                    <button onClick={() => handleDeleteDevice(device.id)} style={{ background: '#dc3545', color: 'white', border: 'none', padding: '5px 10px', borderRadius: '4px', cursor: 'pointer' }}>
                                        Hapus
                                    </button>
                                </td>
                            </tr>
