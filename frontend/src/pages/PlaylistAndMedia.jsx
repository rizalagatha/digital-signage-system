...
const SortablePlaylistItem = React.memo(function SortablePlaylistItem({ id, item, index, onDurationChange, onRemoveItem, onPreviewClick }) {
    ...
    const style = {
        ...
        padding: '5px', border: '1px solid #f0f0f0', backgroundColor: 'white',
    };
    ...
    return (
        ...
        <div ref={setNodeRef} style={style} {...attributes}>
            ...
            <span {...listeners} style={{ padding: '8px', cursor: 'grab', touchAction: 'none' }}>
                ⠿
            </span>

            <div onClick={() => onPreviewClick(item)} style={{ display: 'flex', alignItems: 'center', gap: '10px', flexGrow: 1, cursor: 'pointer' }}>
                <div style={{ width: '60px', height: '40px', backgroundColor: 'black', flexShrink: 0 }}>
                    {item.type === 'image' ? <img src={item.url} loading="lazy" alt={item.filename} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <video src={item.url} preload="metadata" muted loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                </div>
                <span style={{ fontSize: '12px' }}>{item.filename}</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                {item.type === 'image' && (
                    <input type="number" value={item.duration} onChange={(e) => onDurationChange(index, e.target.value)} style={{ width: '60px' }} />
                )}
                {/* 2. TOMBOL DELETE TIDAK PERLU LAGI e.stopPropagation() */}
                <button onClick={() => onRemoveItem(item.id)} style={{ width: 'auto', backgroundColor: '#6c757d', padding: '2px 8px' }}>X</button>
            </div>
        </div>
    );
});
...
function PlaylistAndMedia() {
    ...
    const handleCreatePlaylist = async () => {
        if (!newPlaylistName) return toast.warn('Nama playlist tidak boleh kosong!');
        try {
    ...
    const handleDeletePlaylist = (playlistId) => {
        MySwal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await fetch(`${API_BASE_URL}/api/playlists/${playlistId}`, { method: 'DELETE' });
                    MySwal.fire(
                        'Dihapus!',
    ...
    const handleAddItemToPlaylist = async (mediaId) => {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/api/playlists/${selectedPlaylist.id}/media`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (res.ok) {
                ...
                const updatedPlaylist = newPlaylists.find(p => p.id === selectedPlaylist.id);
    ...
    const handleRemoveMediaFromPlaylist = (mediaId) => {
        ...
        MySwal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await fetch(`${API_BASE_URL}/api/playlists/${selectedPlaylist.id}/media/${mediaId}`, { method: 'DELETE' });
        ...
        return (
            <div style={style}>
                <div style={{ padding: '10px' }}>
                    <div style={{ border: '1px solid #ddd', padding: '10px', borderRadius: '8px', position: 'relative' }}>
                        <button
                        ...
                        <div onClick={() => openPreview(item)} style={{ width: '150px', height: '100px', backgroundColor: '#000', marginBottom: '10px', cursor: 'pointer' }}>
                            {item.type === 'image' ? (
                        ...
                        <p style={{ fontSize: '12px', wordBreak: 'break-all', height: '32px' }}>{item.filename}</p>
                        <button onClick={() => handleAddItemToPlaylist(item.id)} style={{ width: '100%' }}>+ Tambah</button>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '30px' }}>
                <div className="playlist-column" style={{ background: 'white', padding: '20px', borderRadius: '8px' }}>
                    <h2>Manajemen Playlist</h2>
                    <form
                        onSubmit={handleCreatePlaylist}
                        style={{ marginBottom: '20px', padding: '15px', border: '1px solid #ccc', borderRadius: '8px' }}
                    >
                        <input
                            type="text"
                        ...
                        <button type="submit" style={{ marginLeft: '10px', width: '60px' }}>Buat</button>
                    </form>
                    <div className="playlist-list">
                        ...
                        {filteredPlaylists.map(pl => (
                            <div key={pl.id} onClick={() => setSelectedPlaylist(pl)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px', border: '1px solid #eee', marginBottom: '5px', cursor: 'pointer', backgroundColor: selectedPlaylist?.id === pl.id ? '#e0f7fa' : 'white' }}>
                                <span>{pl.name} ({pl.Media ? pl.Media.length : 0} item)</span>
                                <button onClick={() => handleDeletePlaylist(pl.id)} style={{ width: 'auto', backgroundColor: '#dc3545', marginLeft: '10px', padding: '2px 8px' }}>X</button>
                            </div>
                        ))}
                    </div>
                    <hr style={{ margin: '20px 0' }} />
                    <h3>Isi Playlist: {selectedPlaylist ? selectedPlaylist.name : '(Pilih playlist)'}</h3>
                    <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                        <SortableContext items={playlistItems} strategy={verticalListSortingStrategy}>
                    ...
                    {selectedPlaylist && (
                        <button onClick={handleSaveChanges} style={{ width: '100%', marginTop: '20px', padding: '10px' }}>Simpan Perubahan Playlist</button>
                    )}
                </div>

                {/* === KOLOM KANAN: UPLOAD & GALERI MEDIA === */}
                <div className="media-column">
                    <div className="upload-section" style={{ background: 'white', padding: '20px', borderRadius: '8px', marginBottom: '30px' }}>
                        <h3>Upload Media Baru</h3>
                        <input type="file" onChange={handleFileChange} />
                        <button onClick={handleUpload} style={{ marginLeft: '10px', width: 'auto' }}>Upload</button>
                    </div>

                    <div className="gallery-section" style={{ background: 'white', padding: '20px', borderRadius: '8px' }}>
                        <h2>Galeri Media</h2>
                        <input type="text" placeholder="Cari media..." value={mediaSearch} onChange={(e) => setMediaSearch(e.target.value)} style={{ padding: '8px', width: '97%', marginBottom: '20px' }} />
                ...
                <div className="modal-overlay" onClick={closePreview}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        {previewingMedia.type === 'image' ? (
                            ...
                            <video key={previewingMedia.id} src={previewingMedia.url} controls autoPlay muted />
                        )}
                    </div>
                </div>
            )}
